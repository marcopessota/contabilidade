...
if($SOFT){
	...
	$count = $_M->balance_sheet->count($regexs);
	if( $count >0 ) {
		$total_pages = ceil($count/$limit);
	} else {
		$total_pages = 0;
	...
	$result = $_M->balance_sheet->find($regexs)->skip($start)->limit($limit)->sort($sort);

	$i = 0;
	foreach($result as $k => $v){
		$response->rows[$i]['id'] = $v["_id"];
		$response->rows[$i]['cell'] = array($v['desc_account'], $v['chart_account'], $v['debt_sum'], $v['credit_sum'], $v['outstanding_balance_sum']);
		$i++;
	}
