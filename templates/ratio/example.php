...
if($SOFT){
	...
	$count = $_M->sped_teste3->count($regexs);
	if( $count >0 ) {
		$total_pages = ceil($count/$limit);
	} else {
		$total_pages = 0;
	...
	$result = $_M->sped_teste3->find($regexs)->skip($start)->limit($limit)->sort($sort);

	$i = 0;
	foreach($result as $k => $v){
		$response->rows[$i]['id'] = $v["_id"];
		$response->rows[$i]['cell'] = array($v['accountt'], $v['value'],$v['hist']);
		$i++;
	}
